...
CPU routines have the same signature `void(double[:,:],double[:,:],double[:,:],int)`, where the arrays are $A$,$B$ and $C$, while the integer is the dimension of a tile for the filed algorithms. The signature is the same simply because it was convenient, however, non-tiled routines do not make use of the tile size. 
...
Difficulties here stem from the fact that choosing the best block size is not easy and strongly depends on the architecture (cache size, associativity etc.). This, together with the fact that there are three more `for` loops and more complex boundary checking, can make this method slower than the previous. 
...
(I even tried plain C implementations with memory alignment and explicit avx512 instructions, but the improvement over loop reordering was not much at all :( )
...
### GPU blocked
The idea is pretty much the same as for the CPU version, only this time we have unlimited threads. The whole point here is once again improve cache efficiency, which can be done by using CUDA shared memory arrays. Indeed, threads in the same block can access an L1 cache which is much faster than the global shared memory. We can exploit this cache by creating shared arrays that fit in cache and contain blocks of $A$ and $B$, and use these to compute the partial result in a block of $C$. We iterate through blocks until the full result is computed. 

The parallel capabilities of GPUs make this blocked algorithm much more useful and efficient with respect to the CPU couterpart.
...
We can see that the GPU routine is the fastest, followed by the two CPU routines, which are comparable, and finally the serial routine. Curiously, for the smallest size the MPI routine is slower than the serial, possibly due to resources contention between processes. The shared memory routine is slightly faster than the MPI version, which again is possibly due to resource contention. 
...
## Conclusion
The point of this work was not to implement routines that compete with BLAS or CuBLAS, but rather to experiment with tools that can help in writing high-performance Python code, that can be deployed on HPC facilities. We have seen that with these tools we can speed up plain Python code, compile it into CUDA kernels and implement effective distributed algorithms. We have also seen that, although high-level profiling is available, getting low-level performance indicators is not so easy, and it can sometimes be a limitation. Finally, numerical experiments have shown that this framework allows to get good performance with minimal effort.
